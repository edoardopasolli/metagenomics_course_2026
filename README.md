...
path="/home/tools/anaconda3course/bin/"
source ${path}/activate
```

#### With the following command, you get a list of all the environments available in Conda
```
conda info --envs
```
...
```
human_gen_path="/home/projects/course_2026/human_genome/"
conda deactivate
...
conda create -n mpa -c conda-forge -c bioconda python=3.7 metaphlan=4.2.4
...
```
mpa_db="/home/projects/course_2026/metaphlan_databases/"
...
## WE CAN THEN GENERATE THE MERGED TABLE WITH THE VIRAL PROFILES:
...
## Downloading the Nanopore sequence data and running MetaPhlAn
...
## Extracting markers (generated by MetaPhlAn)
```
...
See the R script and associated files in the subfolder [https://github.com/edoardopasolli/metagenomics_course_2026/tree/main/5_panphlan-Rcode](5_panphlan-Rcode)
...
conda create -n humann -c bioconda python=3.7
source ${path}/activate humann
pip install humann==4.0.0a1
...
humann_databases --download uniref uniref90_ec_filtered_diamond humann_dbs --update-config yes
...
## WE ALSO NEED TO CONSIDER TWO CUSTOM SCRIPTS AVAILABLE IN THIS GITHUB REPOSITORY:
filter_contigs.py
megahit2spades.py
```
...
flye --nano-raw ../2_metaphlan/zymo/Zymo-GridION-EVEN-BB-SN.fq.gz --out-dir zymo.flye_asm --meta --threads 8
```
...
## LET'S RUN CHECKM2:
checkm2 predict -i SRR341725_bins -o SRR341725_checkm2 -x .fa --threads 8
...
database_folder="/home/projects/course_2026/phylophlan_databases/" ## THE DB WILL BE AUTOMATICALLY DOWNLOADED HERE WHEN RUNNING PHYLOPHLAN THE FIRST TIME
...
bakta_db="/home/projects/course_2026/bakta_db/db"

cd SRR341725_bins_filtered
mkdir -p tmp
s="bin.8"
bakta --db ${bakta_db} --min-contig-length 200 --prefix ${s} --output ${s}_bakta/ --compliant --meta --threads 8 --tmp-dir ./tmp ${s}.fa
```


### End of Lecture 8 - MAG reconstruction
### Lecture 9 - Statistical analysis
See the R script here in subfolder [https://github.com/edoardopasolli/metagenomics_course_2026/tree/main/9_statistical-analysis-Rcode](9_statistical-analysis-Rcode)
