---
title: "Introduction to metagenomics 2026 - PanPhlAn"
...
- Here we install the third-party packages that we need later

```{r}
install_if_missing <- function(packages, from = "cran"){
  if(!from %in% c("cran", "bioconductor")){
    stop("Unknown from source: use 'cran' or 'bioconductor'.")
  }
...
install_if_missing(bioconductor_packages, from = "bioconductor")
install_if_missing(cran_packages, from = "cran")
```
...
pdf("heatmap_erectale.pdf", width = 8, height = 6)  # Adjust size as needed
draw(heatmap)
dev.off()
```

## MDS (Multidimensional Scaling) plot to visualize PanPhlAn profiles
...
# Create the MDS plot
p <- ggplot(mds_data, aes(x = MDS1, y = MDS2, color = Country)) +
...
p
ggsave("mds_erectale.pdf", plot = p, width = 8, height = 6)
```
